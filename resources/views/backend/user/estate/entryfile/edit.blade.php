...
<div class="page-content">
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
        ...
        <div class="d-flex align-items-center flex-wrap text-nowrap">
            <button type="button" class="btn btn-primary btn-icon-text mb-md-0" data-bs-toggle="modal" data-bs-target="#addMouza">
                                ...
                                <div class="row">
                                    <div class="col-sm-3">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="file_no" value="{{$entryFile->file_no}}" class="form-control" placeholder="Entry File No">
                                        </div>
                                    </div>
    ...
    <!-- More Mouzas Modal -->
    <div class="modal fade" id="addMouza" tabindex="-1" aria-labelledby="mouzaModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="mouzaModalLabel">Add or Update More Mouza</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="btn-close"></button>
                </div>
                <form class="form-horizontal" id="mouzaForm" method="post" action="{{route('user.entryFile.extmouza.store')}}">
                    @csrf
                    @method('PUT')
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="mb-3">
                                    <label class="form-label">Mouza 2 </label>
                                    <select class="form-select mb-3" name="mouza_2">
                                        <option selected disabled>Select Mouza</option>
                                        @foreach(App\Models\Mouza::all() as $mouzas)
                                        <option value="{{$mouzas->id}}" {{$mouzas->id == $entryFile->mouza_2 ? 'selected' :''}}>{{$mouzas->name}} </option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <div class="mb-3">
                                    <label class="form-label"> Mouza 3</label>
                                    <select class="form-select mb-3" name="mouza_3">
                                        <option selected disabled>Select Mouza</option>
                                        @foreach(App\Models\Mouza::all() as $mouzas)
                                        <option value="{{$mouzas->id}}" {{$mouzas->id == $entryFile->mouza_3 ? 'selected' :''}}>{{$mouzas->name}} </option>
                                        @endforeach
                                    </select>
                                </div>
                                <input type="hidden" name="entry_file" value="{{$entryFile->id}}">
                            </div>
                        </div>
