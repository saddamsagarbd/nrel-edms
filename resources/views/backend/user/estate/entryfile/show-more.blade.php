...
<div class="page-content">
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
        ...
        <div class="d-flex align-items-center flex-wrap text-nowrap">
            ...
            <button type="button" class="btn btn-success btn-icon-text mb-2 mb-md-0 ms-1" data-bs-toggle="modal" data-bs-target="#mutationEntryModal">
                <i class="btn-icon-prepend" data-feather="check"></i>
                Mutation
            </button>
        </div>
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href="#" data-deed_id="{{$deed->id}}" class="btn btn-light btn-sm btn-sm-custom editEntryDeed"><i data-feather="edit" width="18" height="18"></i></a>
                                            </td>
                                        </tr>
                                        @endforeach
    ...
    <div class="modal fade" id="registryEntryModal" tabindex="-1" aria-labelledby="registryModallLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content rounded-0">
                ...
                <form id="registryEntryFileForm" method="post">
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="mb-3">
                                    <label class="form-label">Deed Type *</label>
                                    <select class="form-select rounded-0 mb-3" name="deed_type" id="deedType">
                            ...
                            <div class="col-sm-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select rounded-0 mb-3" name="buyer" id="buyerId">
                            ...
                            <input type="hidden" name="deed_id" id="deedId">
...
<script>
    ...
    var entry_file_id = document.getElementById('entryFileId').value;

    $('.dag_livesearch').select2({
        ajax: {
            url: "{{ url('entryfile/dag/search') }}",
    ...
    $(function() {
        ...
        $('#registryEntryBtn').click(function(e) {
            ...
            $(this).html('Sending..');
            $.ajax({
                url: "{{route('user.entryFile.deed.store')}}",
        ...
        $('body').on('click', '.editEntryDeed', function() {
            var deed_id = $(this).data('deed_id');
            $.get("{{ url('user/entryfile/deed') }}" + '/' + deed_id + '/edit', function(data) {
                $('#registryModallLabel').html("Edit Deed ("+data.id+')');
                $('#registryEntryBtn').html("Update");
                $('#registryEntryModal').modal('show');
                $('#deedId').val(data.id);
                $('#dalilNo').val(data.deed_no);
                $('#deed_khatian_land').val(data.dland_size);
                $('#land_value').val(data.land_value);
                $('#registryCost').val(data.expenses);
                $('#registry_date').val(data.deed_date);
                $('#registryOffice').val(data.reg_office);
                $('#deedType option[value="'+data.deed_type+'"]' ).attr('selected', true);
                $('#buyerId option[value="'+data.buyer+'"]' ).attr('selected', true);
            })
        });

        $('#registryEntryModal').on('hidden.bs.modal', function () {
            $('#registryEntryModal form#registryEntryFileForm')[0].reset();
            $('#registryModallLabel').html("Registry The Entry File");
        });
